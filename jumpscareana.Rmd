...
```
  ...
  <!-- Como sou curioso, quero verificar a relação entre o total de filmes e de jumpscares num ano qualquer. Para isso, eu irei normalizar as duas novas variáveis através de uma função criada a partir da seguinte fórmula: $$\frac{x_i - \min(x)}{\max(x) - \min(x)}$$. 
...
#   group_by(year) %>% 
#   summarize(
#     total_jumps = sum(jump_count),
#     total_movies = n()
#   ) %>% 
#   mutate(
#     n_jump = normalize(total_jumps), 
#     n_movies = normalize(total_movies)
#   ) %>% 
#   ggplot(aes(x = year)) + 
#   geom_line(aes(y = n_jump), color = 'blue', size = 1.5) +
#   geom_line(aes(y = n_movies), color = 'red', size = 1.5)
```
-->
